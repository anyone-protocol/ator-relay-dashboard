...
self.onmessage = async (event) => {
  ...
  switch (task) {
    ...
    case 'viewState':
      ...
      await executeTaskWithTimeout(
        () => fetchViewState(contractUrl, payload.functionName),
        task,
        payload.contractName,
        'viewState',
        payload.functionName
    ...
    case 'claimable':
      ...
      await executeTaskWithTimeout(
        () => fetchClaimable(contractUrl, payload.address),
        task,
        payload.contractName,
        'claimable'
      );
...
async function executeTaskWithTimeout(
  ...
  // Create a timeout to terminate if it exceeds 15 seconds
  const timeoutPromise = new Promise((_, reject) =>
    setTimeout(() => {
      reject(`Task timed out after 15 seconds: ${task}`);
    }, MAX_TASK_DURATION)
  );
...
async function fetchClaimable(contractUrl, address) {
  try {
    const response = await fetch(contractUrl);
    const result = await response.json();
    const claimableValue = result.state?.claimable?.[address] || '0';
