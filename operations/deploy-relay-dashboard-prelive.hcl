job "deploy-relay-dashboard-prelive" {
  ...
  group "deploy-relay-dashboard-prelive-group" {
    count = 1

    task "deploy-relay-dashboard-prelive-task" {
      driver = "docker"
      ...
      template {
        data = <<-EOF
        {{ with secret "kv/live-protocol/deploy-relay-dashboard-prelive" }}[r2]
        type = s3
        provider = Cloudflare
        region = auto
