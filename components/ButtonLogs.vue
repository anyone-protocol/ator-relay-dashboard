...
<template>
  <UTooltip text="View network logs">
    <UButton @click="isOpen = true" aria-label="View logs" variant="ghost">
      <Icon name="eos-icons:monitoring" class="text-xl text-cyan-600 dark:text-cyan-300" />
    </UButton>
  </UTooltip>
  ...
  <USlideover v-model="isOpen">
    <div class="px-6 py-3 h-full flex flex-col">
      ...
      <header class="mb-4 flex items-center justify-between">
        ...
        <UButton @click="isOpen = false" aria-label="Close logs" variant="ghost">
          <Icon name="heroicons:x-mark" class="w-6 h-6 dark:text-white" />
        </UButton>
      </header>

      <div class="text-sm mb-6 flex items-center gap-4 rounded-3xl bg-teal-700/10 px-4 py-2 ring ring-cyan-800/60">
        <span>Filter:</span>
      ...
      <div class="relative overflow-auto h-full no-scrollbar mt-4">
        <div class="grid grid-cols-2">
          <div class="py-3.5 pr-3 text-left text-sm font-semibold lg:table-cell">
            Time
          </div>
        ...
        <div class="max-w-full">
          <div class="grid grid-cols-2 dark:text-gray-400" v-for="log in logs.filtered.slice(0, 10)" :key="log.timestamp">
            <div :class="getTextColor(log.level)" class="py-2 pr-3 text-sm lg:table-cell">
              <code> {{ new Date(log.timestamp).toUTCString() }}</code>
            </div>
            <div :class="getTextColor(log.level)" class="py-2 text-sm lg:table-cell">
