...
export const FAMILY_NOT_SET =
  'Subsequent relay claims require family to be set';
...
export class RelayRegistryContract extends Evolvable(Object) {
  ...
  claimable(
    ...
    if (address) {
      ...
      return {
        state,
        result: Object.keys(state.claimable).filter(
          (fp) => state.claimable[fp] === address
        ),
      };
    }
  ...
  isClaimable(
    ...
    return {
      state,
      result:
        Object.keys(state.claimable).includes(fingerprint) &&
        state.claimable[fingerprint] === address,
    };
  }
  ...
  claim(
    ...
    if (state.familyRequired) {
      const verifiedFingerprintsBelongingToCaller = Object.keys(
        state.verified
      ).filter((fp) => state.verified[fp] === caller);
      const claimingRelayFamily = (state.families[fingerprint] || []).slice(0);
      const claimingFamilyIncludesAllVerifiedRelays =
        verifiedFingerprintsBelongingToCaller.every((cf) =>
          claimingRelayFamily.includes(cf)
        );
      ContractAssert(claimingFamilyIncludesAllVerifiedRelays, FAMILY_NOT_SET);
      for (let i = 0; i < verifiedFingerprintsBelongingToCaller.length; i++) {
        ContractAssert(
          state.families[verifiedFingerprintsBelongingToCaller[i]].includes(
            fingerprint
          ),
          FAMILY_NOT_SET
        );
  ...
  verified(
    ...
    if (address) {
      ...
      return {
        state,
        result: Object.keys(state.verified).filter(
          (fp) => state.verified[fp] === address
        ),
      };
    }
    ...
    for (const { fingerprint, add, remove } of families) {
      ...
      if (remove) {
        ...
        for (const removeFingerprint of remove) {
          ...
          const indexToRemove =
            state.families[fingerprint].indexOf(removeFingerprint);
...
export function handle(
  ...
  switch (action.input.function) {
    ...
    case 'evolve':
      return contract.evolve(state, action) as HandlerResult<
        RelayRegistryState,
        any
      >;
    default:
